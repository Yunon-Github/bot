...
logger_original.Logger("ohlcv")

TICK_RES = 5 # OHLCV データの解像度(1分足 : 60, 5分足 : 300, 10分足 : 600, ...)
...
linregLength = 55  # 線形回帰曲線幅
slopeLength = 48  # 傾きの平均幅
signalLength = 27  # シグナル幅
...
def print_ohlcv(rtOHLCV):
  ...
  # 線形回帰曲線の傾きを求める（最小二乗法を使って1次関数の傾きを算出）
  x = n
