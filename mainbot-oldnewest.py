...
def receive_trades():
    ...
    def order_in_bitFlyer(order_type):
        ...
        if order_type == 'Long':
        	...
        	#成行買注文
            btc_Long_taker = api.sendchildorder(product_code='FX_BTC_JPY',
                                                    child_order_type='MARKET',
                                                    side='BUY',
                                                    size=LOGIC_TYPE_LONG_LOT,
            ...
            #成行買注文後のエラーチェック
            r = error_checker(btc_Long_taker)
            if r == 'ok':
                print('注文成功')
            elif r == 'ng':
                print('ロジックを抜けます')
            ...
            try:
                while True:
                    ...
                    print(position)
                    if position == []:
                        positioncheck_count += 1
                        if positioncheck_count > 150:
                        ...
                        btc_Short_maker = api.sendchildorder(product_code='FX_BTC_JPY',
                                                    child_order_type='LIMIT',
                                                    side='SELL',
                                                    size=LOGIC_TYPE_LONG_LOT,
                                                    price=order_maker_price,
                                                    minute_to_expire=1000,
                                                    time_in_force='GTC')
                        ...
                        #指値売注文後のエラーチェック
                        r = error_checker(btc_Short_maker)
                        if r == 'ok':
                            print('注文成功')
                        elif r == 'ng':
                            print('指値注文失敗')
                        ...
                        try:
                            ...
                            while True:
                                #本当に指値注文が完了したかの確認
                                orders_really_check = api.getchildorders(product_code='FX_BTC_JPY' ,child_order_state='ACTIVE')
                                api_count[time_count[0]] += 1

                                #成行注文が残っているか確認
                                position_really_check = api.getpositions(product_code='FX_BTC_JPY')
                                api_count[time_count[0]] += 1
        ...
        #Shortから入る場合
            ...
            #成行売注文
            btc_Short_taker = api.sendchildorder(product_code='FX_BTC_JPY',
                                                    child_order_type='MARKET',
                                                    side='SELL',
                                                    size=LOGIC_TYPE_SHORT_LOT,
                                                    minute_to_expire=1000,
                                                    time_in_force='GTC')
            ...
            #成行売注文後のエラーチェック
            r = error_checker(btc_Short_taker)
            if r == 'ok':
                print('注文成功')
            elif r == 'ng':
                print('ロジックを抜けます')
            ...
            try:
                ...
                while True:
                    ...
                    if position == []:
                        ...
                        btc_Long_maker = api.sendchildorder(product_code='FX_BTC_JPY',
                                                    child_order_type='LIMIT',
                                                    side='BUY',
                                                    size=LOGIC_TYPE_SHORT_LOT,
                                                    price=order_maker_price,
                        ...
                        #成行売注文後のエラーチェック
                        r = error_checker(btc_Long_maker)
                        if r == 'ok':
                            print('注文成功')
                        elif r == 'ng':
                            print('指値注文失敗')
        ...
        try:
            ...
            while True:
                ...
                position_recheck = api.getpositions(product_code='FX_BTC_JPY')
                api_count[time_count[0]] += 1

                orders = api.getchildorders(product_code='FX_BTC_JPY' ,child_order_state='ACTIVE')
                api_count[time_count[0]] += 1

                if position_recheck == [] and orders == []:
                    print('トレードが完了しました')
                    break
                ...
                #ポジションが残っている場合
                    ...
                    kairi = ticker['ltp'] - position_price_result
                    ...
                    if order_type == 'Long':
                        ...
                        if (kairi < LOGIC_TYPE_LONG_KAIRI) or (moving_losscut_flag1 == 1 and kairi > 0):
                            ...
                            try:
                                ...
                                print(e)
                                line_Notify_exec(e, '成行注文の手仕舞いLot確認')


                            #成行注文キャンセル（手仕舞い）
                            tejimai_Long = api.sendchildorder(product_code='FX_BTC_JPY',
                                                    child_order_type='MARKET',
                                                    side='SELL',
                                                    size=tejimai_lot,
                                                    minute_to_expire=1000,
                        ...
                        if kairi > LOGIC_TYPE_SHORT_KAIRI or (moving_losscut_flag1 == 1 and kairi < 0):
                            ...
                            #成行買注文（手仕舞い）
                            tejimai_Short = api.sendchildorder(product_code='FX_BTC_JPY',
                                                    child_order_type='MARKET',
                                                    side='BUY',
                                                    size=tejimai_lot,
                                                    minute_to_expire=1000,
...
#今回未使用
def store_trades_by_ws(trades):
    for trade in trades:
        prc_trades.append({
            'ex_key': trade['ex_key'],
            'size': trade['size'],
            'price': trade['price'],
            'rcv_time': trade['rcv_time'],
        })
